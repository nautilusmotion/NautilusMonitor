...
namespace NautilusMotion.Monitor
{
    ...
    public static class Loc
    {
        ...
        private static readonly Dictionary<string, string[]> S = new Dictionary<string, string[]>
        {
            ...
            {"card.diskhealth", new[]{"DISK HEALTH","SALUD DE DISCO","СОСТОЯНИЕ ДИСКА","SANTÉ DU DISQUE","DATENTRÄGER-ZUSTAND","SAÚDE DO DISCO"}},
            {"health.ok", new[]{"Healthy","Saludable","В норме","Sain","Fehlerfrei","Saudável"}},
            {"health.warn", new[]{"Warning","Advertencia","Предупреждение","Avertissement","Warnung","Aviso"}},
            {"health.bad", new[]{"At risk","En riesgo","Под угрозой","À risque","Gefährdet","Em risco"}},
            {"health.unknown", new[]{"Unknown","Desconocido","Неизвестно","Inconnu","Unbekannt","Desconhecido"}},
            {"dh.reading", new[]{"Reading disk health...","Leyendo la salud del disco...","Чтение состояния диска...","Lecture de la santé du disque...","Datenträger-Zustand wird gelesen...","Lendo a saúde do disco..."}},
            {"dh.wear", new[]{"Wear","Desgaste","Износ","Usure","Verschleiß","Desgaste"}},
            {"dh.needadmin", new[]{
                "Temperature, hours and wear need advanced sensors (administrator).",
                "La temperatura, las horas y el desgaste necesitan los sensores avanzados (administrador).",
                "Температура, часы и износ требуют расширенных датчиков (администратор).",
                "La température, les heures et l'usure nécessitent les capteurs avancés (administrateur).",
                "Temperatur, Stunden und Verschleiß benötigen erweiterte Sensoren (Administrator).",
                "A temperatura, as horas e o desgaste precisam dos sensores avançados (administrador)."}},
